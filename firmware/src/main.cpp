...
<div class="dodo-wrap">
  <div class="dodo-mascot pixel-art calm" id="dodo-pixel">
    <svg viewBox="0 0 32 36" xmlns="http://www.w3.org/2000/svg" shape-rendering="crispEdges">
      ...
      <!-- DISTRESS OVERLAYS (tint + sweat drop, hidden unless .distress) -->
      <rect class="tint" x="0" y="0" width="32" height="36" fill="#9da010" opacity="0.32"/>
      <g class="sweat">
        <rect x="4" y="5" width="1" height="1" fill="#7dd3fc"/>
        <rect x="3" y="6" width="2" height="1" fill="#7dd3fc"/>
        <rect x="3" y="7" width="3" height="1" fill="#7dd3fc"/>
        <rect x="4" y="8" width="2" height="1" fill="#3b82f6"/>
      </g>
      </g>
...
async function refreshData(){
  try{const r=await fetch(DATA_URL);const d=await r.json();
    ...
    const dodoPx=document.getElementById('dodo-pixel');
    if(dodoPx){
...
function initDodiTimelines(){
  const dodi=document.getElementById('dodo-pixel');
  if(!dodi)return;
