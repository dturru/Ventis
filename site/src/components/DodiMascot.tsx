// Reactive Dodi for the demo — front-facing pose ported from the device
// (app/src/components/DodiMascot.tsx), with the GSAP logic replaced by CSS:
// breathing bob, emotion eyes (calm/alert/distress), wing flap when the fan
// runs, and a shake under distress. No look-around side views (kept simple).
...
export function DodiMascot({ emotion, flapping, size = 48 }: Props) {
  const cls = ['dodi-anim', emotion, flapping ? 'flapping' : ''].filter(Boolean).join(' ')
  return (
    <div className={cls} style={{ width: size, height: size * 1.125, flexShrink: 0 }}>
      <svg viewBox="0 0 32 36" xmlns="http://www.w3.org/2000/svg" shapeRendering="crispEdges"
        style={{ width: '100%', height: '100%', display: 'block', overflow: 'visible' }}
        role="img" aria-label="Dodi, the Ventis mascot">
        {/* Wing down (resting) */}
        <g className="dodi-wing-down">
        ...
        {/* Wing up (raised — shown on flap) */}
        <g className="dodi-wing-up">
